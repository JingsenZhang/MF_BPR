...
class BPRData(data.Dataset):
	...
	def ng_sample(self):
		...
		self.features_fill = []
		for x in self.features:                         #feature为train_data
			u, i = x[0], x[1]                           #正样本i
			for t in range(self.num_ng):                #每个用户的一个（u，i）都选取num_ng个负样本j
				j = np.random.randint(self.num_item)    #随机选取一个不在用户物品表（未产生过行为）的物品j作为负样本
				while (u, j) in self.train_mat:
					j = np.random.randint(self.num_item)
				self.features_fill.append([u, i, j])    #三元组训练集[u,i,j]

	def __len__(self):
		return self.num_ng * len(self.features) if \
				self.is_training else len(self.features)

	def __getitem__(self, idx):                        #取数据集中的各个元素
		features = self.features_fill if \
				self.is_training else self.features    #训练集取三元组，测试集取二元组
