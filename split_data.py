...
def split_basic(path='./data/ratings.csv',per=0.8):
...
def split_basic_v2(path='./data/ratings.csv',per=0.8):
    data = pd.read_csv(path, header=None, names=['user', 'item','rate','time'], sep=',')
    ...
    trainset.to_csv("data/train_basic.csv", header=None, index=False)          # 写入csv
